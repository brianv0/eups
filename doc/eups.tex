...
Post-tags are useful when you want exact products, but you don't want to have to search your email for
the name of currently-blessed version, e.g. \code{afw 6.1.3.1}.  You might think of tagging it with
\code{rhl}: \code{eups declare -t rhl afw 6.1.3.1; setup -t rhl afw} and that would work until you
tagged some dependent product \code{rhl} --- at that point you'd get both of your tagged products.  The
solution is to make the tag only apply if no version is known, in other words put the tag \textit{after}
\code{version} in the \code{vro}.  And this is exactly what \code{-T rhl} does.
...
\end{verbatim}
and then tell her to repeat that last line.  If she wants to use some of her bug-fixed versions in conjunction
with my versions, she'd say \code{setup -t lynda -t mysetups.txt afw} which is why this approach is often
better than just sourcing a set of \code{setup -j XXX} lines.

\subsubsection{Specifying default Tags}
\label{defaultTags}

If you don't specify any tags to the \code{setup} (or \code{eups vro}) command (with either \code{--tag} (\code{-t}) or \code{--postTag} (\code{-T})) then
\code{hooks.config.Eups.defaultTags} is consulted.  It's a dictionary with keys \code{pre} and \code{post};
e.g. if you have
\begin{verbatim}
hooks.config.Eups.defaultTags["pre"] += ["rhl", "HSC"]
hooks.config.Eups.defaultTags["post"] += ["v6_1"]
\end{verbatim}
in \file{startup.py}, then \code{setup foo} is exactly equivalent to \code{setup -t rhl,HSC -T v6\_1}.
You can disable this default tag processing with \code{--tag None} (or \code{--tag ""}).
...
Due to restrictions on the server side (e.g. \textbf{robots.txt} files) checking all the possible tags
can be annoyingly slow, and it's especially annoying if you have specified an explicit tag.  You might
want to consider using \code{--no-server-tags} (equivalent to \code{-U}) to avoid this; the specified
tag will still be installed.

You sometimes want to run some code after products have been installed.  For example, due to restrictions
imposed by os/x (``The SIP'') you may need to fix up the \texttt{\#!} (``shebang'') lines, or to patch symbolic links for
shared object files that point outside the installed tree.  The mechanism is that if a file
\file{distribInstallPostHook.py} is found in one of the standard places to find customisation files (as shown
by \code{eups startup -v}). There is an example file in the \eups source directory, \file{callback/distribInstallPostHook.py} which patches both shebangs and shared object file paths.

This file needs to be on the \textit{installer}'s machine, of course,
e.g. \file{/u/products/site/distribInstallPostHook.py}.
...
You can list tags with e.g. \code{eups distrib tags w\_2017*} (note the glob; csh users in particular
will probably need to protect it from the shell).

Note that there isn't enough information in any single file on the 
server to tell us which products are available for which flavor, so
to get accurate listings we have to do another round-trip query for
each product/version/flavor.  This is slow, so is only implemented
if you specify \code{eups distrib list --precise} (or \code{--verbose} --- but that'll also
...
Because this list of pre-defined variables is so inflexible, you can define a dictionary
of your own values in a startup file (\textit{e.g.} \code{\$HOME/.eups/startup.py}). For
example, after
\begin{verbatim}
hooks.config.distrib["builder"]["variables"]["WHO"] = "RHL"
\end{verbatim}
I can refer to \code{@WHO@} in build files.
...
\end{verbatim}
which allow the insertion of centrally controlled shell functions into all build files as they
are expanded.
...
Options:
   ...
   -S glob, --showTags=glob Only show tags that match this glob
...
\eups can be customised using files in a surprisingly large number of places; \code{eups startup}
lists the startup files that are actually used (with \code{-v} non-existent files that would be
...
generated by a command such as  \code{eups declare -t XXX -r .} or \code{eups declare -t XXX productName} where you didn't name a version}.  You can force the underlying version to be undeclared along with the tag by specifying \code{--undeclareVersion} (or equivalently \code{--U}), e.g. \code{eups undeclare scons -t w\_2017\_52 -U}.
...
\begin{itemize}
   ...
   \item \code{envUnset(<Name>)}
     It might sound as if this directive unsets the environment variable \code{<Name>}, and it
     would be possible to implement this if you have a use for it.  However, at present the
     only supported \code{<envUnset>} command is
     \code{envUnset(PRODUCT\_DIR)} which may be used to prevent
     \eups from setting \code{\$PRODUCT\_DIR}.
     For example, \code{git} uses \code{GIT\_DIR} for its own purposes,
     but yet it's useful to use \eups to manage \code{git} versions.

   \item \code{print(<String>)}
   \item \code{print(<Dest>, <String>)}
     Print the string to stdout; if \code{<Dest>} is provided it must be one of
     \code{stdout}, \code{stderr}, \code{stdok}, \code{stdinfo}, and \code{stdwarn}.
...
\end{verbatim}
   ...
   \item \code{condaActivate(<EnvName>)}
     Emits a \code{conda activate <EnvName>} for switching to a conda environment.
     Conda must be activated first and the environment must be present.

   \item \code{condaActivateStack(<EnvName1> [, <EnvName2>...])}
     Emits a \code{conda activate --stack <EnvName1>, ...} for activating stacked
     conda environments. Conda must be activated first and the environment must be
     present.
